...
[2026-07-20] Decision: URL oficial de produccion confirmada - `https://app-venta.vercel.app/`. El usuario decidio no comprar dominio; esta URL de Vercel sera la oficial para operar el evento. Verificado desde navegador el 2026-07-20: la pagina responde y muestra selector de sede (UNIDEP Tampico / Parque Mendez).
[2026-07-20] Decision: pulido visual premium final - se creo PRODUCT.md con registro product, se definio sistema visual claro para uso en celular bajo sol (fondo blanco, verde dinero/ok, amarillo aviso, rojo urgente, azul navegacion), se redisenaron selector de sede, header, nav inferior con iconos lucide, Vender/Ticket/Cobro, Inflable, Resumen, Caja y Ajustes con botones mas grandes, paneles consistentes, inputs legibles y estados vacios claros. Verificado con npm run lint PASS, npm run build PASS y capturas Playwright en viewport movil 390x844; captura final de Inflable sin traslapes relevantes (overlay negro observado es Next dev, no produccion).
[2026-07-20] Decision: preview premium para compartir link - se agregaron metadatos Open Graph/Twitter con metadataBase `https://app-venta.vercel.app`, imagen social generada en `src/app/opengraph-image.tsx` (1200x630 PNG) y `robots.txt` cambio de Disallow a Allow para que WhatsApp/Facebook puedan leer el preview; la app sigue con `robots.index=false` para no indexarse. Verificado con `npm run lint` PASS y `npm run build` PASS.
[2026-07-21] Decision: excepcion explicita a la regla de "orden fijo, nunca por popularidad" (seccion 5.2) - a pedido directo del usuario, Vender (`src/app/(tabs)/vender/page.tsx`) ahora ordena por categoria (bebida arriba, comida abajo via `src/lib/categoriaProductos.ts`, clasificacion fija por nombre) y dentro de cada categoria los agotados van al final. Esto sigue siendo un orden determinista y estable (no cambia segun cuanto se venda cada producto), no es "popularidad" en el sentido que la seccion 5.2 prohibe. Una auditoria externa (Codex) marco esto como violacion del contrato sin tener este contexto - queda documentado aqui para que no se vuelva a reportar como bug. Tambien de esta fecha: fotos de producto emparejadas por nombre (`src/lib/imagenesProductos.ts`, sin tocar el esquema de Supabase) y visibilidad de productos por sede en Vender (`productosAsignadosSede()` en `src/lib/stock.ts` - cada sede solo ve productos que alguna vez recibieron inventario ahi; uno agotado por ventas normales sigue mostrandose como "Agotado", no desaparece).
[2026-07-22] Decision: correcciones tras auditoria "quirurgica" de Codex (7 bloqueantes, 5 graves, 1 menor reportados) - se corrigieron: limite silencioso de 1000 filas de PostgREST en Resumen/Caja/stock (nuevo helper `src/lib/seleccionarTodo.ts` que pagina con `.range()`), estados de error compartidos entre fetches independientes que se ocultaban entre si en Ajustes e Inflable (ahora usan `Promise.allSettled` y solo limpian el error si TODOS los fetches tuvieron exito), extension de inflable (`extenderTiempo` en `src/lib/tiempos.ts`) ahora lee el estado real del timer antes de escribir (rechaza extender uno ya terminado) y revierte los minutos si falla el insert de la venta, alta de producto (`crearProducto`) ahora borra el producto si falla el insert de inventario inicial (evita huerfanos), y se bloquearon costo/precio/stock negativos en alta y edicion de producto. Se dejaron sin tocar, por decision explicita del usuario: (1) misma-ultima-pieza-vendida-dos-veces (B-001) - riesgo aceptado a proposito desde el spec original (INSERT sin bloqueos entre las 2 sedes, stock queda en rojo como aviso), (2) RLS deshabilitado / anon key con acceso total - decision ya documentada arriba, no es hallazgo nuevo, (3) edicion concurrente de producto con "gana el ultimo" - bajo impacto real. Pendiente con el usuario: duplicado de venta si se pierde la respuesta de red justo tras un insert exitoso (B-002) - requiere una columna nueva de idempotencia en `ventas` via SQL que el usuario debe correr el mismo (no hay llave de servicio/DDL disponible para el agente); el codigo que la usa se agrega en cuanto confirme que corrio el `ALTER TABLE`.
